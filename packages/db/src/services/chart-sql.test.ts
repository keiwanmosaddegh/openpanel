...
describe('overview.service / getRawWhereClause (game_key resolved filter)', () => {
  ...
  // added by 18-materialize-hot-event-property-keys and 19-materialize-game-tag
  // (named in full: upstream ships its own 18 and 19). The local dev CH schema
  // doesn't carry them, so a real-table EXPLAIN can't run locally.
  // Parsing/execution was validated directly on prod (drill-down == Top Games
  // row for boxr over a cutover-spanning window).
});
